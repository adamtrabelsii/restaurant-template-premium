    ...
    <footer className="bg-ardor-mid pt-16 pb-8">
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-12 mb-16">
        ...
        <div>
          ...
          <p className="font-cormorant italic text-ardor-muted text-base mb-6">{t('footer.tagline')}</p>
          <div className="flex gap-4">
            {SOCIAL.map(s => (
              <motion.a
                key={s.label}
                href="#"
                aria-label={socialAriaTpl.replace('{network}', s.label)}
        ...
        {/* Navigation */}
        <div>
          <h4 className="font-montserrat text-xs tracking-[0.4em] uppercase text-ardor-red mb-6">{t('footer.navigateHeading')}</h4>
        ...
        {/* Contact */}
        <div>
          <h4 className="font-montserrat text-xs tracking-[0.4em] uppercase text-ardor-red mb-6">{t('footer.findUsHeading')}</h4>
          ...
          <div className="mt-6 font-montserrat text-xs text-ardor-muted space-y-1">
            <p><span className="text-white">{t('footer.hours.weekdays.label')}</span> · {t('footer.hours.weekdays.value')}</p>
            <p><span className="text-white">{t('footer.hours.weekend.label')}</span> · {t('footer.hours.weekend.value')}</p>
            <p><span className="text-white">{t('footer.hours.sunday.label')}</span> · {t('footer.hours.sunday.value')}</p>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <p className="font-montserrat text-xs text-ardor-muted">{t('footer.copyright')}</p>
        <p className="font-montserrat text-xs text-ardor-muted">{t('footer.crafted')}</p>
      </div>
    </footer>
  )
}
